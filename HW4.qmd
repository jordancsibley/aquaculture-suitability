...
# Create map 
tm_shape(depth_range) + # depth raster to show borders 
...
tm_shape(eez_oyster) +  # eez zones 
  tm_polygons(col = "zone_pct",
              palette = "Oranges", 
              alpha = 0.5,
              title = "% of suitable area \nper EEZ (km2)") + 
  tm_credits("Oregon", position = c(0.25, 0.7), size = 0.6) +
...
tm_shape(suitable_oyster) + # suitable oyster area
  ...
  tm_compass(position = c(0.03, 0.12),
             size = 1.5) 
```



- Change color palette of the zones 
- Label each zone instead of legend 
