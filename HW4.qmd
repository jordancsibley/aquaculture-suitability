...
suitable_oyster <- terra::lapp(oyster_stack[[c(1,2)]], fun = function(depth, sst) depth * sst)
...
# Step 3: Mask the reclassified raster by the eez zones (this will keep only the suitable areas)
#suitable_mask <- suitable_oyster * eez_rast

# Set 0 values to NA 
#suitable_mask[suitable_mask == 0] <- NA

tm_shape(eez_rast) +
  tm_raster()

# Calculate area of grid cells of eez? 
eez_area <- cellSize(eez_rast, unit = "m")




# Step 4: Calculate the total area for each zone using zonal statistics
# zone_area <- terra::zonal(masked_rast, eez_rast, fun = "sum", na.rm = TRUE)

# Step 5: Convert cell counts to area (area of one raster cell)
#cell_area <- terra::res(suitable_area_rast)[1] * terra::res(suitable_area_rast)[2]

# Step 6: Calculate total area (sum of suitable areas * area of one cell)
#zone_area$total_area <- zone_area$sum * cell_area
