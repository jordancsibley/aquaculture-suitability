...
### Remove land from depth 
```{r}
# Any value that is greater than depth(elevation) = 0 turns to NA
depth[depth > 0] <- NA
...
rcl_depth <- matrix(c(0, -70, 1, # values 0 to 70 = 1
                      -70, -Inf, 0), # values 70 to Inf = 0
                    ncol = 3, byrow = TRUE)
...
# Create raster stack of temp and depth
depth_temp_oyster <- c(depth_oyster, sst_oyster)

# Visualize reclassification 
depthmap <- tm_shape(depth_temp_oyster[[1]]) +
  tm_raster() 
  
tempmap<- tm_shape(depth_temp_oyster[[2]]) +
  tm_raster()
...
Lets investigate if the land can be removed 
```{r}
# turn anything that is greater than equal to 0 for depth NA 
